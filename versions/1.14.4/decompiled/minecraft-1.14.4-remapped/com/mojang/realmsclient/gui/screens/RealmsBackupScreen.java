...
public class RealmsBackupScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      (new Thread("Realms-fetch-backups") {
         public void run() {
            ...
            try {
               List<Backup> var2 = var1.backupsFor(RealmsBackupScreen.this.serverData.id).backups;
               Realms.execute(() -> {
                  ...
                  for(Backup var3 : RealmsBackupScreen.this.backups) {
   ...
   private void addToChangeList(Backup backup, String string) {
      if(string.contains("Uploaded")) {
         ...
         backup.changeList.put(string, string);
   ...
   private void restoreClicked(int selectedBackup) {
      if(selectedBackup >= 0 && selectedBackup < this.backups.size() && !this.serverData.expired) {
         ...
         Date var2 = ((Backup)this.backups.get(selectedBackup)).lastModifiedDate;
         String var3 = DateFormat.getDateTimeInstance(3, 3).format(var2);
         String var4 = RealmsUtil.convertToAgePresentation(Long.valueOf(System.currentTimeMillis() - var2.getTime()));
   ...
   private void downloadWorldData() {
      RealmsTasks.DownloadTask var1 = new RealmsTasks.DownloadTask(this.serverData.id, this.slotId, this.serverData.name + " (" + ((RealmsWorldOptions)this.serverData.slots.get(Integer.valueOf(this.serverData.activeSlot))).getSlotName(this.serverData.activeSlot) + ")", this);
   ...
   class BackupObjectSelectionList extends RealmsObjectSelectionList {
      ...
      public void selectItem(int selected) {
         ...
         if(selected != -1) {
            Realms.narrateNow(RealmsScreen.getLocalizedString("narrator.select", new Object[]{((Backup)RealmsBackupScreen.this.backups.get(selected)).lastModifiedDate.toString()}));
         }
   ...
   class BackupObjectSelectionListEntry extends RealmListEntry {
      ...
      public BackupObjectSelectionListEntry(Backup mBackup) {
      ...
      private void renderBackupItem(Backup backup, int var2, int var3, int var4, int var5) {
         int var6 = backup.isUploadedVersion()?-8388737:16777215;
         RealmsBackupScreen.this.drawString("Backup (" + RealmsUtil.convertToAgePresentation(Long.valueOf(System.currentTimeMillis() - backup.lastModifiedDate.getTime())) + ")", var2 + 40, var3 + 1, var6);
         RealmsBackupScreen.this.drawString(this.getMediumDatePresentation(backup.lastModifiedDate), var2 + 40, var3 + 12, 5000268);
