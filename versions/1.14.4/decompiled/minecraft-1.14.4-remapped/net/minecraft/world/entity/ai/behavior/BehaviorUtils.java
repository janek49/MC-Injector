...
public class BehaviorUtils {
   ...
   public static boolean targetIsValid(Brain brain, MemoryModuleType memoryModuleType, EntityType entityType) {
      return brain.getMemory(memoryModuleType).filter((livingEntity) -> {
         return livingEntity.getType() == entityType;
      }).filter(LivingEntity::isAlive).filter((livingEntity) -> {
   ...
   public static void throwItem(LivingEntity var0, ItemStack itemStack, LivingEntity var2) {
      double var3 = var0.y - 0.30000001192092896D + (double)var0.getEyeHeight();
      ItemEntity var5 = new ItemEntity(var0.level, var0.x, var3, var0.z, itemStack);
   ...
   public static SectionPos findSectionClosestToVillage(ServerLevel serverLevel, SectionPos var1, int var2) {
      int var3 = serverLevel.sectionsToVillage(var1);
      Stream var10000 = SectionPos.cube(var1, var2).filter((sectionPos) -> {
         return serverLevel.sectionsToVillage(sectionPos) < var3;
      });
      serverLevel.getClass();
      return (SectionPos)var10000.min(Comparator.comparingInt(serverLevel::sectionsToVillage)).orElse(var1);
   }
}
