...
public class RingBell extends Behavior {
   ...
   protected void start(ServerLevel serverLevel, LivingEntity livingEntity, long var3) {
      ...
      if(var6.closerThan(new BlockPos(livingEntity), 3.0D)) {
         BlockState var7 = serverLevel.getBlockState(var6);
         if(var7.getBlock() == Blocks.BELL) {
            BellBlock var8 = (BellBlock)var7.getBlock();

            for(Direction var10 : Direction.Plane.HORIZONTAL) {
               if(var8.onHit(serverLevel, var7, serverLevel.getBlockEntity(var6), new BlockHitResult(new Vec3(0.5D, 0.5D, 0.5D), var10, var6, false), (Player)null, false)) {
                  break;
               }
            }
         }
