...
public class Schedule {
   ...
   protected static ScheduleBuilder register(String string) {
      Schedule var1 = (Schedule)Registry.register(Registry.SCHEDULE, (String)string, new Schedule());
   ...
   protected List getAllTimelinesExceptFor(Activity activity) {
      return (List)this.timelines.entrySet().stream().filter((map$Entry) -> {
         return map$Entry.getKey() != activity;
      }).map(Entry::getValue).collect(Collectors.toList());
   }

   public Activity getActivityAt(int i) {
      return (Activity)this.timelines.entrySet().stream().max(Comparator.comparingDouble((map$Entry) -> {
         return (double)((Timeline)map$Entry.getValue()).getValueAt(i);
