...
public class Illusioner extends SpellcasterIllager implements RangedAttackMob {
   ...
   public Illusioner(EntityType entityType, Level level) {
         ...
         if(this.hurtTime != 1 && this.tickCount % 1200 != 0) {
            if(this.hurtTime == this.hurtDuration - 1) {
               ...
               for(int var1 = 0; var1 < 4; ++var1) {
                  ...
                  this.clientSideIllusionOffsets[1][var1] = new Vec3(0.0D, 0.0D, 0.0D);
               }
            }
         } else {
            this.clientSideIllusionTicks = 3;
   ...
   public Vec3[] getIllusionOffsets(float f) {
      if(this.clientSideIllusionTicks <= 0) {
         ...
         double var2 = (double)(((float)this.clientSideIllusionTicks - f) / 3.0F);
         var2 = Math.pow(var2, 0.25D);
         Vec3[] vars4 = new Vec3[4];

         for(int var5 = 0; var5 < 4; ++var5) {
            vars4[var5] = this.clientSideIllusionOffsets[1][var5].scale(1.0D - var2).add(this.clientSideIllusionOffsets[0][var5].scale(var2));
         }
   ...
   class IllusionerBlindnessSpellGoal extends SpellcasterIllager.SpellcasterUseSpellGoal {
      ...
      public boolean canUse() {
         return !super.canUse()?false:(Illusioner.this.getTarget() == null?false:(Illusioner.this.getTarget().getId() == this.lastTargetId?false:Illusioner.this.level.getCurrentDifficultyAt(new BlockPos(Illusioner.this)).isHarderThan((float)Difficulty.NORMAL.ordinal())));
      }
