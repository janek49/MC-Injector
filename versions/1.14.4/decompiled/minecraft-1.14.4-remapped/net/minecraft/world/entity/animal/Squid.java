...
public class Squid extends WaterAnimal {
   ...
   public Squid(EntityType entityType, Level level) {
   ...
   public void aiStep() {
      ...
      if(this.isInWaterOrBubble()) {
         ...
         this.yBodyRot += (-((float)Mth.atan2(var1.x, var1.z)) * 57.295776F - this.yBodyRot) * 0.1F;
         this.yRot = this.yBodyRot;
         this.zBodyRot = (float)((double)this.zBodyRot + 3.141592653589793D * (double)this.rotateSpeed * 1.5D);
         this.xBodyRot += (-((float)Mth.atan2((double)var2, var1.y)) * 57.295776F - this.xBodyRot) * 0.1F;
      } else {
         this.tentacleAngle = Mth.abs(Mth.sin(this.tentacleMovement)) * 3.1415927F * 0.25F;
         if(!this.level.isClientSide) {
            double var1 = this.getDeltaMovement().y;
            if(this.hasEffect(MobEffects.LEVITATION)) {
               var1 = 0.05D * (double)(this.getEffect(MobEffects.LEVITATION).getAmplifier() + 1);
            } else if(!this.isNoGravity()) {
               var1 -= 0.08D;
            }

            this.setDeltaMovement(0.0D, var1 * 0.9800000190734863D, 0.0D);
         }
   ...
   public boolean hurt(DamageSource damageSource, float var2) {
      if(super.hurt(damageSource, var2) && this.getLastHurtByMob() != null) {
         this.spawnInk();
         return true;
      } else {
         return false;
   ...
   private Vec3 rotateVector(Vec3 vec3) {
      Vec3 var2 = vec3.xRot(this.xBodyRotO * 0.017453292F);
   ...
   public static boolean checkSquidSpawnRules(EntityType entityType, LevelAccessor levelAccessor, MobSpawnType mobSpawnType, BlockPos blockPos, Random random) {
   ...
   class SquidFleeGoal extends Goal {
      ...
      public boolean canUse() {
         ...
         return Squid.this.isInWater() && var1 != null?Squid.this.distanceToSqr(var1) < 100.0D:false;
      }
