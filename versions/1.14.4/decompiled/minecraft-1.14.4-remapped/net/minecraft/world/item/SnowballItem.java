...
public class SnowballItem extends Item {
   public SnowballItem(Item.Properties item$Properties) {
   ...
   public InteractionResultHolder use(Level level, Player player, InteractionHand interactionHand) {
      ...
      level.playSound((Player)null, player.x, player.y, player.z, SoundEvents.SNOWBALL_THROW, SoundSource.NEUTRAL, 0.5F, 0.4F / (random.nextFloat() * 0.4F + 0.8F));
      if(!level.isClientSide) {
         Snowball var5 = new Snowball(level, player);
         var5.setItem(var4);
