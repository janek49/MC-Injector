...
public enum RegionHillsLayer implements AreaTransformer2, DimensionOffset1Transformer {
   ...
   public int applyPixel(Context context, Area var2, Area var3, int var4, int var5) {
      ...
      if(var6 > 255) {
         LOGGER.debug("old! {}", Integer.valueOf(var6));
      }
      ...
      if(!Layers.isShallowOcean(var6) && var7 >= 2 && var8 == 1) {
         ...
         if(var9 == null || !var9.isMutated()) {
            Biome var10 = Biome.getMutatedVariant(var9);
            return var10 == null?var6:Registry.BIOME.getId(var10);
         }
      }

      if(context.nextRandom(3) == 0 || var8 == 0) {
         int var9 = var6;
         if(var6 == DESERT) {
            var9 = DESERT_HILLS;
         } else if(var6 == FOREST) {
            ...
            var9 = BIRCH_FOREST_HILLS;
         } else if(var6 == DARK_FOREST) {
            var9 = PLAINS;
         } else if(var6 == TAIGA) {
            var9 = TAIGA_HILLS;
         } else if(var6 == GIANT_TREE_TAIGA) {
            var9 = GIANT_TREE_TAIGA_HILLS;
         } else if(var6 == SNOWY_TAIGA) {
            var9 = SNOWY_TAIGA_HILLS;
         } else if(var6 == PLAINS) {
            var9 = context.nextRandom(3) == 0?WOODED_HILLS:FOREST;
         } else if(var6 == SNOWY_TUNDRA) {
            var9 = SNOWY_MOUNTAIN;
         } else if(var6 == JUNGLE) {
            var9 = JUNGLE_HILLS;
         } else if(var6 == BAMBOO_JUNGLE) {
            var9 = BAMBOO_JUNGLE_HILLS;
         } else if(var6 == Layers.OCEAN) {
            var9 = Layers.DEEP_OCEAN;
         } else if(var6 == Layers.LUKEWARM_OCEAN) {
            var9 = Layers.DEEP_LUKEWARM_OCEAN;
         } else if(var6 == Layers.COLD_OCEAN) {
            var9 = Layers.DEEP_COLD_OCEAN;
         } else if(var6 == Layers.FROZEN_OCEAN) {
            var9 = Layers.DEEP_FROZEN_OCEAN;
         } else if(var6 == MOUNTAINS) {
            var9 = WOODED_MOUNTAINS;
         } else if(var6 == SAVANNA) {
            var9 = SAVANNA_PLATEAU;
         } else if(Layers.isSame(var6, WOODED_BADLANDS_PLATEAU)) {
            var9 = BADLANDS;
         } else if((var6 == Layers.DEEP_OCEAN || var6 == Layers.DEEP_LUKEWARM_OCEAN || var6 == Layers.DEEP_COLD_OCEAN || var6 == Layers.DEEP_FROZEN_OCEAN) && context.nextRandom(3) == 0) {
            var9 = context.nextRandom(2) == 0?PLAINS:FOREST;
         }

         if(var8 == 0 && var9 != var6) {
            Biome var10 = Biome.getMutatedVariant((Biome)Registry.BIOME.byId(var9));
            var9 = var10 == null?var6:Registry.BIOME.getId(var10);
         }
