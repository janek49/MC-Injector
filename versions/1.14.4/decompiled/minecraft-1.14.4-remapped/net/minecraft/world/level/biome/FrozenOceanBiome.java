...
public final class FrozenOceanBiome extends Biome {
   ...
   protected float getTemperatureNoCache(BlockPos blockPos) {
      ...
      double var3 = FROZEN_TEMPERATURE_NOISE.getValue((double)blockPos.getX() * 0.05D, (double)blockPos.getZ() * 0.05D);
      double var5 = BIOME_INFO_NOISE.getValue((double)blockPos.getX() * 0.2D, (double)blockPos.getZ() * 0.2D);
      double var7 = var3 + var5;
      if(var7 < 0.3D) {
         double var9 = BIOME_INFO_NOISE.getValue((double)blockPos.getX() * 0.09D, (double)blockPos.getZ() * 0.09D);
         if(var9 < 0.8D) {
            var2 = 0.2F;
         }
      }
      ...
      if(blockPos.getY() > 64) {
         float var9 = (float)(TEMPERATURE_NOISE.getValue((double)((float)blockPos.getX() / 8.0F), (double)((float)blockPos.getZ() / 8.0F)) * 4.0D);
         return var2 - (var9 + (float)blockPos.getY() - 64.0F) * 0.05F / 30.0F;
      } else {
         return var2;
      }
   }
