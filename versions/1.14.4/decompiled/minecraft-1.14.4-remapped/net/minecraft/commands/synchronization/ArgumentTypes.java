...
public class ArgumentTypes {
   ...
   public static void register(String string, Class class, ArgumentSerializer argumentSerializer) {
      ResourceLocation var3 = new ResourceLocation(string);
      if(BY_CLASS.containsKey(class)) {
         throw new IllegalArgumentException("Class " + class.getName() + " already has a serializer!");
      } else if(BY_NAME.containsKey(var3)) {
         throw new IllegalArgumentException("\'" + var3 + "\' is already a registered serializer!");
      } else {
         ArgumentTypes.Entry<T> var4 = new ArgumentTypes.Entry(class, argumentSerializer, var3);
         BY_CLASS.put(class, var4);
   ...
   public static void serialize(FriendlyByteBuf friendlyByteBuf, ArgumentType argumentType) {
      ArgumentTypes.Entry<T> var2 = get(argumentType);
      if(var2 == null) {
         ...
         friendlyByteBuf.writeResourceLocation(var2.name);
         var2.serializer.serializeToNetwork(argumentType, friendlyByteBuf);
      }
   }

   @Nullable
   public static ArgumentType deserialize(FriendlyByteBuf friendlyByteBuf) {
      ResourceLocation var1 = friendlyByteBuf.readResourceLocation();
      ArgumentTypes.Entry<?> var2 = get(var1);
      if(var2 == null) {
         LOGGER.error("Could not deserialize {}", var1);
         return null;
      } else {
         return var2.serializer.deserializeFromNetwork(friendlyByteBuf);
      }
   }

   private static void serializeToJson(JsonObject jsonObject, ArgumentType argumentType) {
      ArgumentTypes.Entry<T> var2 = get(argumentType);
      if(var2 == null) {
         ...
         jsonObject.addProperty("parser", var2.name.toString());
         JsonObject var3 = new JsonObject();
         var2.serializer.serializeToJson(argumentType, var3);
         if(var3.size() > 0) {
            jsonObject.add("properties", var3);
         }
      }

   }

   public static JsonObject serializeNodeToJson(CommandDispatcher commandDispatcher, CommandNode commandNode) {
      ...
      for(CommandNode<S> var5 : commandNode.getChildren()) {
         var3.add(var5.getName(), serializeNodeToJson(commandDispatcher, var5));
      }

      if(var3.size() > 0) {
         jsonObject.add("children", var3);
      }
      ...
      if(commandNode.getRedirect() != null) {
         Collection<String> var4 = commandDispatcher.getPath(commandNode.getRedirect());
         if(!var4.isEmpty()) {
   ...
   static class Entry {
      ...
      private Entry(Class clazz, ArgumentSerializer serializer, ResourceLocation name) {
         this.clazz = clazz;
