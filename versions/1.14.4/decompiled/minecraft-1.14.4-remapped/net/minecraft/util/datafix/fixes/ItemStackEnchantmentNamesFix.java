...
public class ItemStackEnchantmentNamesFix extends DataFix {
   private static final Int2ObjectMap MAP = (Int2ObjectMap)DataFixUtils.make(new Int2ObjectOpenHashMap(), (int2ObjectOpenHashMap) -> {
   ...
   public ItemStackEnchantmentNamesFix(Schema schema, boolean var2) {
      super(schema, var2);
   }
   ...
   private Dynamic fixTag(Dynamic dynamic) {
      Optional var10000 = dynamic.get("ench").asStreamOpt().map((stream) -> {
         return stream.map((dynamic) -> {
            return dynamic.set("id", dynamic.createString((String)MAP.getOrDefault(dynamic.get("id").asInt(0), "null")));
         });
      });
      dynamic.getClass();
      Optional<Dynamic<?>> var2 = var10000.map(dynamic::createList);
      if(var2.isPresent()) {
         dynamic = dynamic.remove("ench").set("Enchantments", (Dynamic)var2.get());
      }
