...
public class BiomeFix extends DataFix {
   ...
   public BiomeFix(Schema schema, boolean var2) {
      super(schema, var2);
   }

   protected TypeRewriteRule makeRule() {
      Type<Pair<String, String>> var1 = DSL.named(References.BIOME.typeName(), DSL.namespacedString());
      if(!Objects.equals(var1, this.getInputSchema().getType(References.BIOME))) {
         throw new IllegalStateException("Biome type is not what was expected.");
      } else {
         return this.fixTypeEverywhere("Biomes fix", var1, (dynamicOps) -> {
            return (pair) -> {
               return pair.mapSecond((string) -> {
                  return (String)BIOMES.getOrDefault(string, string);
               });
            };
         });
      }
