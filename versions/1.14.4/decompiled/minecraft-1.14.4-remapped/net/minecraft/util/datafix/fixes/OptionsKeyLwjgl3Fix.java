...
public class OptionsKeyLwjgl3Fix extends DataFix {
   ...
   public OptionsKeyLwjgl3Fix(Schema schema, boolean var2) {
      super(schema, var2);
   }
            ...
            return (Dynamic)dynamic.getMapValues().map((map) -> {
               return dynamic.createMap((Map)map.entrySet().stream().map((map$Entry) -> {
                  if(((Dynamic)map$Entry.getKey()).asString("").startsWith("key_")) {
                     int var1 = Integer.parseInt(((Dynamic)map$Entry.getValue()).asString(""));
                     if(var1 < 0) {
                        int var2 = var1 + 100;
                        String var3;
                        if(var2 == 0) {
                           ...
                           var3 = "key.mouse." + (var2 + 1);
                        }

                        return Pair.of(map$Entry.getKey(), ((Dynamic)map$Entry.getValue()).createString(var3));
                     } else {
                        String var2 = (String)MAP.getOrDefault(var1, "key.unknown");
                        return Pair.of(map$Entry.getKey(), ((Dynamic)map$Entry.getValue()).createString(var2));
                     }
                  } else {
