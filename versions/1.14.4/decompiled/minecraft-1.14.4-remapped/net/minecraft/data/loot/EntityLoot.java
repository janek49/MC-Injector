...
public class EntityLoot implements Consumer {
   private static final EntityPredicate.Builder ENTITY_ON_FIRE = EntityPredicate.Builder.entity().flags(EntityFlagsPredicate.Builder.flags().setOnFire(Boolean.valueOf(true)).build());
   private static final Set SPECIAL_LOOT_TABLE_TYPES = ImmutableSet.of(EntityType.PLAYER, EntityType.ARMOR_STAND, EntityType.IRON_GOLEM, EntityType.SNOW_GOLEM, EntityType.VILLAGER);
   ...
   public void accept(BiConsumer biConsumer) {
      ...
      Set<ResourceLocation> var2 = Sets.newHashSet();
      ...
      while(true) {
         ...
         if(!SPECIAL_LOOT_TABLE_TYPES.contains(var4) && var4.getCategory() == MobCategory.MISC) {
            ...
            if(var6 == null) {
               throw new IllegalStateException(String.format("Missing loottable \'%s\' for \'%s\'", new Object[]{var5, Registry.ENTITY_TYPE.getKey(var4)}));
            }

            biConsumer.accept(var5, var6);
         }
      }

      throw new IllegalStateException(String.format("Weird loottable \'%s\' for \'%s\', not a LivingEntity so should not have loot", new Object[]{var5, Registry.ENTITY_TYPE.getKey(var4)}));
   }

   private void add(EntityType entityType, LootTable.Builder lootTable$Builder) {
      this.add(entityType.getDefaultLootTable(), lootTable$Builder);
   }

   private void add(ResourceLocation resourceLocation, LootTable.Builder lootTable$Builder) {
      this.map.put(resourceLocation, lootTable$Builder);
   }
