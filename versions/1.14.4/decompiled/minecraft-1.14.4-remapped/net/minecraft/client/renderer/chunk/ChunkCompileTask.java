...
@ClientJarOnly
public class ChunkCompileTask implements Comparable {
   private final RenderChunk chunk;
   private final ReentrantLock lock = new ReentrantLock();
   private final List cancelListeners = Lists.newArrayList();
   ...
   public ChunkCompileTask(RenderChunk chunk, ChunkCompileTask.Type type, double distAtCreation, @Nullable RenderChunkRegion region) {
      this.chunk = chunk;
      this.type = type;
   ...
   public void setBuilders(ChunkBufferBuilderPack builders) {
   ...
   public void cancel() {
      ...
      try {
         ...
         for(Runnable var2 : this.cancelListeners) {
            var2.run();
         }
      } finally {
         this.lock.unlock();
