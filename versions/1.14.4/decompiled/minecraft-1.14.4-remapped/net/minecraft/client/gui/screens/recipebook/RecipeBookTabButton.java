...
public class RecipeBookTabButton extends StateSwitchingButton {
   ...
   public void startAnimation(Minecraft minecraft) {
      ClientRecipeBook var2 = minecraft.player.getRecipeBook();
      List<RecipeCollection> var3 = var2.getCollection(this.category);
      if(minecraft.player.containerMenu instanceof RecipeBookMenu) {
         label49:
         for(RecipeCollection var5 : var3) {
            Iterator var6 = var5.getRecipes(var2.isFilteringCraftable((RecipeBookMenu)minecraft.player.containerMenu)).iterator();
   ...
   public void renderButton(int var1, int var2, float var3) {
      ...
      this.renderIcon(var4.getItemRenderer());
   ...
   private void renderIcon(ItemRenderer itemRenderer) {
      List<ItemStack> var2 = this.category.getIconItems();
      int var3 = this.isStateTriggered?-2:0;
      if(var2.size() == 1) {
         itemRenderer.renderAndDecorateItem((ItemStack)var2.get(0), this.x + 9 + var3, this.y + 5);
      } else if(var2.size() == 2) {
         itemRenderer.renderAndDecorateItem((ItemStack)var2.get(0), this.x + 3 + var3, this.y + 5);
         itemRenderer.renderAndDecorateItem((ItemStack)var2.get(1), this.x + 14 + var3, this.y + 5);
      }
   ...
   public boolean updateVisibility(ClientRecipeBook clientRecipeBook) {
      List<RecipeCollection> var2 = clientRecipeBook.getCollection(this.category);
      this.visible = false;
      if(var2 != null) {
